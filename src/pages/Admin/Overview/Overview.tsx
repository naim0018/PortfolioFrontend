...
const Overview = () => {
  ...
  const { data: portfoliosResp } = useGetPortfoliosQuery(undefined, {
    skip: !isAdmin,
  });
    ...
    <div className="mx-auto space-y-8 p-6 pt-0">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Welcome back, {userProfile?.userName || userProfile?.email}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {stats.map((stat, idx) => (
          ...
          >
            <div
              className={`h-14 w-14 ${stat.color} flex items-center justify-center rounded-2xl`}
            >
              {stat.icon}
            </div>
            <div>
